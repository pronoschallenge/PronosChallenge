...
class EvolutionClassementResource extends Resource {
    ...
    function get($request) {
		...
		echo '$idDerniereJournee = ', $idDerniereJournee, '/n';
		
		// recherche du championnat en cours
		$querySaisonEnCours = "SELECT phpl_gr_championnats.id FROM phpl_gr_championnats WHERE phpl_gr_championnats.activ_prono='1' ORDER by id desc";
		$resultat = mysql_query ($querySaisonEnCours) or die ("probleme " .mysql_error());
		$row = mysql_fetch_array($resultat);		
		$idSaisonEnCours = $row[0];
		echo '$idSaisonEnCours = ', $idSaisonEnCours, '/n';
		
		// Liste des tops / flops
		$queryEvolution = 	"SELECT P1.type, (P1.classement - P2.classement) as evolution, membre.pseudo 
							 ...
							 ORDER BY P1.type, evolution DESC, membre.pseudo";
	    ...
	    while ($row = mysql_fetch_array($resultat))
	    {
			...
			$numEvolution = $row["evolution"];
       		...
       		array_push($data, array("typeChamp" => $typeChamp, "numEvolution" => $numEvolution, "nomPseudo" => $nomPseudo));
		}
